...
<body>
    <div class="wrapper">
        ...
        <section style="font-size: 20px; padding: 20px;">
    <?php 
    ...
    // Assume database connection is stored in $mysql
    $q = "SELECT Admin_id, Admin_Name, Email, Phoneno, Image FROM tbladmin"; 
    $res = mysqli_query($mysql, $q) or die("Failed!!! " . mysqli_error($mysql)); 

    if (mysqli_num_rows($res) > 0) {
        echo "<table cellpadding='10' cellspacing='0' align='center' style='margin-left: 10px; width: 100%; max-width: 1200px;'>";    
        $counter = 0;
        while ($r = mysqli_fetch_array($res)) {
            ...
            <td style='padding: 15px;'>
                <div class='card' style='width: 100%; max-width: 300px; text-align: center; padding: 15px; border-radius: 10px; margin-bottom: 20px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); transition: transform 0.2s;'>
                    <img src='" . htmlspecialchars($r[4]) . "' alt='Admin Image' class='card-img-top' style='width: 100%; height: auto; border-radius: 50%; object-fit: cover; max-width: 150px; margin-bottom: 15px;'>
                    
                    <div class='card-body' style='padding: 10px;'>
                        <h5 class='card-title' style='font-weight: bold; color: #333;'><strong>Admin ID:</strong> " . htmlspecialchars($r[0]) . "</h5>
                        <p class='card-text' style='color: #555;'><strong>Admin Name:</strong> " . htmlspecialchars($r[1]) . "</p>
                        <p class='card-text' style='color: #555;'>
                            <strong>Email:</strong> 
                            <a href='mailto:" . htmlspecialchars($r[2]) . "' style='color: #007bff; text-decoration: none;'>" . htmlspecialchars($r[2]) . "</a>
                        </p>
                        <p class='card-text' style='color: #555;'><strong>Phone No:</strong> " . htmlspecialchars($r[3]) . "</p>
                        
                        <a href='Contact_us.php?aid=" . urlencode($r[0]) . "&dname=" . urlencode($_SESSION['dname']) . "&aname=" . urlencode($r[1]) . "' class='btn btn-primary' style='background-color: #o; border: none; padding: 10px 20px; border-radius: 5px; color: white; font-weight: bold; text-decoration: none; transition: background-color 0.3s;'>
                            Contact Us
                        </a>
                    </div>
                </div>
        ...
        echo "<p class='text-center'>No Admin found.</p>";
    }
    ?>
</section>
...
<script ="assets/vendor/jQuery.countdown-master/jquery.countdown.min.js"></script>
