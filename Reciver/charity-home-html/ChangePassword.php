        ...
        <div class="container" style="margin:200px 430px; width: 100%; height: 100%;">
            <div class="row my-4 my-lg-5">
                <div class="col-12 col-md-8 offset-md-2 col-lg-6 offset-lg-3 text-center">
                    <p class="font-20 semi-font fables-main-text-color mt-4 mb-4 mb-lg-5" style="font-size:50px">Change Password </p>
</br>
                    <form style="height:90px;">
                        <div class="form-group" > 
                            <div class="input-icon">
                            <?php if (isset($error)) { echo "<p class='error'>$error</p>"; } ?>
                                <input type="password" class="form-control rounded-0 py-3 pl-5 font-13 sign-register-input"  placeholder="Enter Old Password" name="opwd"style="height:50px; font-size:20px; font-weight:600;"> 
                            </div>
                        ...
                        <div class="form-group"> 
                            <div class="input-icon">
                                ...
                                <input type="password" class="form-control rounded-0 py-3 pl-5 font-13 sign-register-input" placeholder="Enter New Password" name="pass"style="height:50px; font-size:20px; font-weight:600;">
                            </div>
                        </div> 
                        <div class="form-group"> 
                            <div class="input-icon">

                                <input type="password" class="form-control rounded-0 py-3 pl-5 font-13 sign-register-input" placeholder="Enter confirm Password" name="cpass"style="height:50px; font-size:20px; font-weight:600;">
                            </div>
                        </div> 
                        <button type="submit" class="btn btn-block rounded-0 white-color fables-main-hover-background-color fables-second-background-color font-16 semi-font py-3" style="font-size:20px" name="submit">Submit</button><br><br>
                        </form>
                </div>
            </div>
        </div>
...
if (isset($_REQUEST['submit'])) {
    ...
    if ($newPassword !== $confirmPassword) {
        ...
        $sql = "SELECT * FROM tblreceiver WHERE Receiver_Name = '$Recevier_Name' AND Password = '$oldPassword'";
        $result = mysqli_query($mysql, $sql);

        if (mysqli_num_rows($result) == 1) {
            // Old password is correct, update to the new password
            $updateSql = "UPDATE tblreceiver SET Password = '$newPassword' WHERE Receiver_Name = '$Recevier_Name'";
            if (mysqli_query($mysql, $updateSql)) {
                // Password changed successfully
                echo '<script>alert("Password changed successfully.");</script>';
                // Uncomment the line below to redirect after the password change
